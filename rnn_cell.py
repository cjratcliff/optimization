...
from nn_utils import weight_matrix, bias_vector, xavier_initializer, identity_initializer
...
class BasicRNNCell(RNNCell):
	...
	def __init__(self, num_units, activation=tf.nn.elu):
	...
	def __call__(self, inputs, state, scope=None):
		...
		with tf.variable_scope(scope or type(self).__name__):	# "BasicRNNCell"
			output = self._activation(_linear([inputs, state], self._num_units, bias=True, identity_init=True))
...
def _linear(args, output_size, bias, bias_start=0.0, scope=None, identity_init=False):
	...
	with tf.variable_scope(scope or "Linear"):
		if identity_init:
			matrix = tf.get_variable("Matrix", [total_arg_size, output_size], initializer=identity_initializer([total_arg_size, output_size]))
		else:
